...
oc get serviceaccount,service,job,configmap,deployment,daemonset,statefulset -n "${NAMESPACE}" -l app.kubernetes.io/instance=portworx -o json | \
jq -c '.items[] | {"name": .metadata.name, "kind": .kind}' | \
while read resource; do
  kind=$(echo "${resource}" | jq -r '.kind')
  name=$(echo "${resource}" | jq -r '.name')

  patch=$(jq -n --arg UID "${SERVICE_UID}" --arg API_VERSION "${SERVICE_API_VERSION}" --arg KIND "${SERVICE_KIND}" \
